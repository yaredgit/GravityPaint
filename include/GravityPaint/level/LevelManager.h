...
public:
    ...
    Difficulty getGameDifficulty() const { return m_gameDifficulty; }
...
private:
    ...
    // Screen dimensions for level generation
    int m_screenWidth = 540;
    int m_screenHeight = 960;
    
    // Game difficulty
    Difficulty m_gameDifficulty = Difficulty::Medium;

    // Spawn state (using char instead of bool to avoid std::vector<bool> quirks)
    std::vector<char> m_spawnedObjects;
